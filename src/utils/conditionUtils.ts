...
export const checkRoleGroupCondition = (
  ...
  const relevantGroupIds = seat.groupIds.filter((gid) =>
    condition.groupIds.includes(gid),
  );
...
export const checkStudentDistanceCondition = (
  ...
  // 対象生徒でない場合は常にtrue
  if (
    student.id !== condition.studentId1 &&
    student.id !== condition.studentId2
  )
    return true;
  ...
  const otherStudentId =
    student.id === condition.studentId1
      ? condition.studentId2
      : condition.studentId1;
  const otherStudentSeatId = Object.keys(currentAssignments).find(
    (seatId) => currentAssignments[seatId] === otherStudentId,
  );
...
export const checkAllConditions = (
  ...
  return conditions.every((condition) => {
    switch (condition.type) {
      case "student-group":
        return checkStudentGroupCondition(
      ...
      case "role-group":
        return checkRoleGroupCondition(
      ...
      case "student-distance":
        return checkStudentDistanceCondition(
...
export const calculateConstraintDifficulty = (
  ...
  // 生徒間距離条件の影響を評価
  const distanceConditions = conditions.filter(
    (c) =>
  ...
  // 生徒-グループ条件の影響を評価
  const studentGroupConditions = conditions.filter(
    (c) =>
...
export const sortStudentsByConstraintDifficulty = (
  ...
  return [...students].sort((a, b) => {
    const difficultyA = calculateConstraintDifficulty(
      a,
      seats,
      conditions,
      groups,
    ...
    const difficultyB = calculateConstraintDifficulty(
      b,
      seats,
      conditions,
      groups,
    ...
    switch (condition.type) {
      case "student-group":
        ...
        for (const studentId of studentGroupCondition.studentIds) {
          ...
          const hasTargetGroup = seat.groupIds.some((gid) =>
            studentGroupCondition.groupIds.includes(gid),
          );
      ...
      case "student-distance":
        ...
        const student1SeatId = Object.keys(assignments).find(
          (seatId) =>
            assignments[seatId] === studentDistanceCondition.studentId1,
        );
        const student2SeatId = Object.keys(assignments).find(
          (seatId) =>
            assignments[seatId] === studentDistanceCondition.studentId2,
        );
        ...
        if (student1SeatId && student2SeatId) {
          ...
          if (seat1 && seat2) {
            ...
            const student1Name = students.find(
              (s) => s.id === studentDistanceCondition.studentId1,
            )?.name;
            const student2Name = students.find(
              (s) => s.id === studentDistanceCondition.studentId2,
            )?.name;
            ...
            if (studentDistanceCondition.shouldBeClose && distance > 2) {
              ...
              !studentDistanceCondition.shouldBeClose &&
              distance < 3
            ) {
              satisfied = false;
...
export const generateCSPSeatAssignment = (
  ...
  // 制約の厳しさで生徒をソート
  const sortedStudents = sortStudentsByConstraintDifficulty(
    students,
    seats,
    conditions,
    groups,
  ...
  const backtrack = (
    ...
    const validSeats = remainingSeats.filter((seat) =>
      checkAllConditions(
        ...
        roles,
        sortedStudents,
        assignments as { [seatId: string]: string },
        seats,
      ),
    );
...
export const generateCSPSeatAssignmentWithAnalysis = (
  ...
  const assignment = generateCSPSeatAssignment(
    students,
    seats,
    conditions,
    groups,
