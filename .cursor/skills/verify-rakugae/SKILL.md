...
That clicks the button named `スキップ`. Overlay heading `ラクガエへようこそ！` must disappear. Do not start the 3-minute tour unless the feature file is `onboarding.md`.
...
| `読み込み` / `保存` | buttons | header CSV import/export |
