...
- Choose `名簿を読み込む` to load a roster CSV.
...
- `名簿を読み込む` (`#btn-prep-import-roster`) lives on the 生徒 tab. It opens a CSV format modal, then the locked confirm. `#btn-header-import` / `#btn-header-export` in the header are backup load and save only. Helper `fill` does not attach files unless you extend it; use `choose-file` on the hidden input after the modal 読み込む / バックアップファイルを保存 path. The global tab has no `バックアップ・引き継ぎ` card.
