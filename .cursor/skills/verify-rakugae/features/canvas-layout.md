...
- View mode hides editing tools; switch to `編集` via `#btn-footer-viewmode` on desktop, or `#btn-phone-viewmode` on phone.
- Phone toolbar buttons keep accessible names (`座席を追加`) via `aria-label` even when the visible label is hidden.
