...
- **生徒情報のスワップ（入れ替え）**: `Ctrl`キーを押しながら座席をドラッグし、他の座席の上にドロップした場合は、重なり禁止の挙動にはならず、両方の「座席の生徒情報」のみが入れ替わる（スワップされる）。※「座席ロック」状態の席ではこのスワップ操作はできない。
- **空席と生徒の割り当て（ダブルクリック）**:
...
- **座席のコンテキストメニュー（右クリック）**:
  - 選択状態で右クリックすると、「複製」「削除」が実行できる。
  - （※現在実装中または今後実装: グループ割り当て、座席ロック設定など）
