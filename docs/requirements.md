...
- **印刷**: ブラウザの標準印刷で、座席表を A4 1枚に出す。座席の氏名だけを、はみ出さない範囲でなるべく大きく刷る。
...
- **印刷**: 先生が A4 に座席表を出し、教室の壁に貼るか、机の上で配置を確認する。ブラウザの標準印刷ダイアログだけを使う。印刷面は編集キャンバスの複製ではない。その時点の座席配置から A4 1枚分の座席表を組み直して出す。詳細は 3.8.1 以降。実装の境界は `docs/print-design.md`。

#### 3.8.1 印刷の開始

- フッタの「印刷」（`#btn-footer-print`）を押すと印刷設定を表示し、用途を選んでからブラウザの標準印刷画面を開く。
- 印刷設定は次の 2 種類。初期値は「掲示用」。
  - **掲示用（氏名を正立）**: 教室の壁に貼る。前方（教卓側）が上、氏名は正立。
  - **机上確認用（氏名を 180° 回転）**: 机に置いて向かい側から読む。座席と図形の位置は変えず、氏名だけを各座席の中心で 180° 回す。
- 設定画面には、配置から自動で選ばれた用紙向きを「A4 横（自動）」または「A4 縦（自動）」と表示する。向き自体はユーザーが変えない。
- `Ctrl` / `Cmd` + `P`、およびブラウザメニューからの印刷では、同じタブ内で最後に確定した設定を使う。未確定なら「掲示用」。
- 座席が 1 つもないときは印刷設定を開かず、印刷できる座席がないことを通知する。
- 印刷設定は同じタブを開いている間だけ保持する。LocalStorage、バックアップ、教室データには保存しない。

#### 3.8.2 用紙と配置

- 1 枚の A4 に出す。余白は上下左右 10mm。複数ページへの分割はしない。
- 座席および印刷対象の図形を合わせた外接範囲について、A4 縦と A4 横の双方で収まる最大倍率を算出し、より大きく出せる向きを選ぶ。同率なら横向き。
- 外接範囲全体を余白内の中央に置く。画面の表示リセットにある 100% 上限は印刷には適用しない。小さい座席表は拡大し、大きい座席表は 1 枚に収まるまで縮小する。はみ出しは禁止する。
- 氏名は枠内の中央に収まるよう席ごとに大きさを下げ、省略記号（…）では切らない。
- 画面のパン位置・ズーム倍率・選択状態には依存せず、印刷の前後でもこれらを変更しない。
- ブラウザの印刷ダイアログで余白を広げられた場合は、座席表全体を縮小して 1 枚に収める（切れない）。
- 画面幅 390px を含むコンパクト表示でも、画面幅ではなく A4 の寸法を基準に同じ結果を出す。

#### 3.8.3 印刷する内容

- 座席は白地の単純な枠と、割り当て済み生徒の **氏名だけ** を印刷する。
- 空席は文字を入れず、空の座席枠として印刷する（「空席」は出さない）。
- 出席番号、ふりがな、ロール、グループ色・印、ロック、選択枠、操作部品は印刷しない。
- 汎用オブジェクトは座席位置を読むための目印として図形の輪郭だけを印刷する。オブジェクトの文字（「教卓」など）、色、編集用ハンドルは印刷しない。
- アプリ名、見出し（「ラクガエ 座席表」）、日付、クラス名、凡例その他の文字は追加しない。
- 「机上確認用」でも氏名だけを各座席の中心で 180° 回転する。座席表全体の向きは変えない。全体を 180° 回す選択肢は設けない（余白が対称で見出しが無い紙を裏返すのと同じだから）。
- 印刷は専用の簡略表示を使う。現在が編集モードか生徒閲覧モードかにかかわらず、常に上記と同じ内容を出す。印刷のためにモードを切り替えない。

#### 3.8.4 プライバシーと出力方式

- 印刷処理は完全にブラウザ内で行い、氏名や座席情報を外部へ送信しない。
- PDF 生成ライブラリやアプリ独自の PDF 出力は追加しない。出力先の選択はブラウザの標準印刷画面に委ねる。
- 検証はネイティブの印刷ダイアログを開かない。印刷メディアのエミュレーションで、画面シェルが消え、印刷用の座席表だけが見え、すべての座席が用紙内にあり、「ラクガエ 座席表」「空席」「教卓」が出ず、印刷前後でズーム表示が変わらないことを確認する。

#### 3.8.5 対象外

- 座席表全体の回転、画面上の視点切り替えを印刷で代用すること、用紙サイズ・余白・縦横の手動指定、複数ページ、タイル印刷、アプリ内印刷プレビュー。
- ふりがな・出席番号・グループ色の印刷、見出し・日付・クラス名の追加。
- A4 以外の用紙、PDF / Excel ライブラリ、サーバー経由の印刷・共有、印刷設定の永続化。
- ブラウザやプリンターが独自に付けるヘッダー・フッター、および利用者が標準印刷画面で変更した用紙・倍率はアプリの制御対象外とする（余白を広げられた場合の縮小は 3.8.2）。
...
- 印刷の一時設定（掲示用 / 机上確認用）は AppState・persist・バックアップの対象外。独立した非永続ストアで持つ。
