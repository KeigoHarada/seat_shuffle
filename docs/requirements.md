...
- **空席と生徒の割り当て（ダブルクリック）**:
  ...
  - **空席のダブルクリック**: ダブルクリックした座席のすぐ近くにポップオーバーが表示され、「新規生徒の登録（直接追加）」または「登録済みだが座席未配置の生徒」の一覧が表示される。ポップオーバー内の生徒をクリックすると、即座にその空席へ割り当てられポップオーバーが閉じる。
...
- **座席のコンテキストメニュー（右クリック）**:
  - 選択状態で右クリックすると、「生徒の割り当て解除（生徒配置時のみ）」「複製」「削除」が実行できる。
