...
### 3.11 開発者支援・寄付機能（OFUSE）

教育現場向けに完全無料で提供されている「ラクガエ」の継続的な保守、サーバー維持費、および新機能開発を支援するため、OFUSEを通じた応援・寄付機能を提供する。
メインの席替え操作画面（ヘッダー・フッター・キャンバス）には配置せず、教員が設定確認時に自然にアクセスできるよう「設定（全体設定）パネル」内の「その他」セクションに集約する。

- **配置場所**: 設定パネル > 全体設定（設定タブ） > 「その他」セクション
- **支援プラットフォーム**: **OFUSE (オフセ)**（`https://ofuse.me/o?uid=218335`）
  - ファンレターや応援メッセージを添えて100円から支援可能（PayPay・クレジットカード対応、会員登録不要）
- **デザイン仕様**:
  - ラクガエのブランドトーン＆マナー（温かみのあるパステルイエロー/オレンジ系、角丸デザイン、Zen Maru Gothicフォント）に最適化した専用カードおよびボタンを配置
  - OFUSEウィジェット属性（`data-ofuse-widget-button`, `data-ofuse-id="218335"`, `data-ofuse-size="large"`, `data-ofuse-color="dark-invert"`）を備えた埋め込みリンク
  - ワンクリックでクリップボードにURLをコピーできる「URLコピー」ボタンを提供
  - OFUSE公式ウィジェットスクリプト（`widget.js`）の非同期読み込み対応
