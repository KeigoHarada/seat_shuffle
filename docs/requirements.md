...
- **バックアップ**: 内部プロジェクトの完全な状態をファイルとして保存・読み込む。読み込みは全置換。壊れているファイルは拒否し、現在の状態を残す。画面上では「バックアップ」と呼び、JSONという語は出さない。
...
- **印刷**: ブラウザの標準印刷。画面のツールバー・設定・フッタ操作は印刷しない。座席と短い見出しだけ出す。
...
2. **utils**: 席替え・名簿CSV・バックアップ・班割当などの純粋関数。`src/utils/algorithm.ts` など。React コンポーネントを import しない。
