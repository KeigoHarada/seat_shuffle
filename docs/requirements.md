...
教育現場向けに完全無料で提供されている「ラクガエ」の継続的な保守、サーバー維持費、および新機能開発を支援するため、寄付・応援機能を提供する。

- **アクセスポイント**:
  - ヘッダー右上の「寄付で応援」ボタン（Heartアイコン）
  - フッター左側の「開発者を応援・寄付する」リンクボタン
  - 全体設定パネル内の「開発者を支援・寄付」カード
- **応援コース（目安金額）選択**:
  - ☕ コーヒー1杯コース (¥300): 日々の開発リフレッシュ
  - 🍱 ランチ応援コース (¥1,000): サーバー・ドメイン維持費の補助
  - 🚀 開発ブーストコース (¥3,000): 新機能・アルゴリズム改善の推進
  - 👑 ゴールドサポーター (¥5,000): 長期的な安定運用と継続改善を支援
  - ✏️ 自由金額: 100円以上の任意の金額設定
- **外部寄付・支援プラットフォーム連携**:
  - **OFUSE (オフセ)**: メッセージ・ファンレターを添えた寄付（クレジットカード、PayPay対応、登録不要）
  - **Buy Me a Coffee**: グローバル標準の1クリック支援（Apple Pay、Google Pay、クレジットカード）
  - **GitHub Sponsors**: 開発者 KeigoHarada への直接支援（月額 / 単発）
  - **Stripe 決済**: クレジットカード決済（URL設定時）
  - **Amazon ほしい物リスト / ギフト**: ギフト券等の直接支援（URL設定時）
  - **各プラットフォームURLのワンクリックコピー機能**
- **アプリ内応援メッセージ＆寄付シミュレーション**:
  - ニックネーム、応援メッセージ、金額を入力して送信可能
  - 送信完了時に紙吹雪（Confetti）のアニメーション演出
  - 送信者に「✨ 公認ラクガエサポーター」バッジを付与し、ローカルストレージに履歴を記録
- **サポーター特典・履歴表示**:
  - サポーター認定バッジ、総支援額、過去の応援メッセージ履歴の一覧表示
  - X（旧Twitter）でのシェア機能
- **寄付先URLのカスタマイズ機能**:
  - 管理者・開発者が自身の寄付用URL（OFUSE、Buy Me a Coffee、GitHub Sponsors、Stripe、Amazon、カスタムURL）を自由に設定・保存可能（LocalStorage永続化）
