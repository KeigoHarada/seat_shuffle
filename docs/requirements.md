...
- 印刷設定は次の 2 種類。初期値は「掲示用」。
  ...
  - **机上確認用（文字を 180° 回転）**: 机に置いて向かい側から読む。座席と図形の位置は変えず、各座席の文字だけを座席枠の中心で 180° 回す。
- 設定画面には、印刷対象の外接範囲から自動で選ばれた用紙向きを「A4 横（自動）」または「A4 縦（自動）」と表示する。向き自体はユーザーが変えない。
- `Ctrl` / `Cmd` + `P`、およびブラウザメニューからの印刷では、同じタブ内で最後に確定した設定を使う。未確定なら「掲示用」。印刷対象（全体か選択範囲か）は、ダイアログを開いた時点のキャンバス選択に従う。
...
- 印刷対象の外接範囲について、A4 縦と A4 横の双方で収まる最大倍率を算出し、より大きく出せる向きを選ぶ。同率なら横向き。
...
- 「机上確認用」では座席の文字（出席番号・ふりがな・氏名・「空席」）だけを各座席の中心で 180° 回転する。図形の位置と図形上の文字（「教卓」）は回さない。座席表全体を 180° 回す選択肢は設けない。
...
- ブラウザやプリンターが独自に付けるヘッダー・フッター、および利用者が標準印刷画面で変更した用紙・倍率はアプリの制御対象外とする（余白を広げられた場合の縮小は 3.8.3）。
