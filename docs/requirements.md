...
### 3.11 ブランドデザイン・ロゴ・ファビコン仕様

- **ブランドシンボル**:
  - 教育現場の「座席（机/椅子）」と「シャッフル・回転（チェンジ）」および「楽しさ・ワクワク（スパークル）」を表現した角丸スクエア（Squircle）のSVGシンボルマーク。
  - カラーは Primary Orange（`#F59E0B`）ベースのグラデーションに、座席カードとパステルグループカラー（ブルー/ピンク）のアクセント。
- **ファビコン (Favicon)**:
  - ブラウザタブやブックマーク用に最適化したSVGファビコン（`public/favicon.svg`）および互換アイコンを提供。
  - 極小サイズ（16×16px）でも視認性を損なわないベクター設計。
- **タイトルロゴ (Logo Component)**:
  - シンボルマークとフォント「Zen Maru Gothic」に合わせたブランドタイトル「ラクガエ」およびサブタイトル「席替え支援Webアプリ」を統合したコンポーネント。
  - ヘッダーおよび各種モーダル等で統一されたブランド体験を提供。
