...
- **ターゲット**: 教員のPCが主。電話は座席の提示・シャッフル・名簿と条件の編集。
- **View は Dual-tree**: `src/views/desktop` と `src/views/phone` は chrome を共有しない。ドメイン（store / `useShuffle` / Canvas・名簿タブ）だけ共有する。`compact` や `layout` props で一つのヘッダーを縮めない。
- **電話（幅 767px 以下）**: 下タブで画面を分ける。行き先は「座席」「名簿」「条件」。シャッフルと編集/閲覧は座席画面の操作バー。役割・グループは名簿の中。全体設定・CSV・ガイド・規約はヘッダーのメニュー。PCの左右分割は変えない。
- **キャンバスエリア**: パン・ズーム操作を基本とし、画面サイズに依存せず広々と使える設計にする。電話では教室が収まるまで 25% まで縮小してよい。
- **サイドパネル**: デスクトップでは開閉する。電話では出さない。
- **品質**: `npm run test:phone-layout` が 375 / 390 / 1280 で横溢れ・ツールバー折り返し・名簿での閲覧トグル混入を落とす。
